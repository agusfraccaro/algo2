...
int main(){
    ...
    auxiliar->clave = 1;
    printf("No se puede borrar de un arbol vacío: %s\n", borrar(arbol, auxiliar)==-1?"SI":"NO");
    ...
    cosa* c2sub = crear_cosa(2);
    cosa* c4sub = crear_cosa(4);
    cosa* c6sub = crear_cosa(6);

    insertar(arbol, c4sub);
    insertar(arbol, c2sub);
    insertar(arbol, c6sub);

    free(auxiliar);
    printf("El arbol se destruye correctamente: %s\n", destruir_arbol(arbol)==0?"SI":"NO");
    return 0;
}
